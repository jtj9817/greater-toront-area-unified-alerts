...
function createMiwayFeedMessage(int $cause = Alert\Cause::TECHNICAL_PROBLEM, int $effect = Alert\Effect::DETOUR): string
{
    ...
    $alert1 = new Alert;
    $alert1->setCause($cause);
    $alert1->setEffect($effect);
...
it('falls back to UNKNOWN_* when the feed returns unknown enum values', function () {
    Http::fake([
        'https://www.miapp.ca/gtfs_rt/Alerts/Alerts.pb' => Http::response(createMiwayFeedMessage(9999, 9999), 200),
    ]);

    $service = app(MiwayGtfsRtAlertsFeedService::class);
    $result = $service->fetch();

    expect($result['alerts'][0]['cause'])->toBe('UNKNOWN_CAUSE')
        ->and($result['alerts'][0]['effect'])->toBe('UNKNOWN_EFFECT');
});

it('returns not_modified when 304 is received', function () {
    Carbon::setTestNow(Carbon::parse('2026-03-31 08:00:00', 'America/New_York'));
    $expectedTimestamp = Carbon::now()->timestamp;
    ...
    expect($result)->toHaveKey('not_modified')
        ...
        ->and($result['updated_at']->isUtc())->toBeTrue()
        ->and($result['updated_at']->timestamp)->toBe($expectedTimestamp);
...
it('does not treat body \"0\" as an empty payload', function () {
    config(['feeds.allow_empty_feeds' => false]);

    Http::fake([
        'https://www.miapp.ca/gtfs_rt/Alerts/Alerts.pb' => Http::response('0', 200),
    ]);

    $service = app(MiwayGtfsRtAlertsFeedService::class);
    $service->fetch();
})->throws(RuntimeException::class, 'Failed to decode MiWay GTFS-RT protobuf payload');
