...
/**
 * Manual Test: YRT Phase 4 Queue Job Wrapper + Scheduler
 * Generated: 2026-04-01
 * Purpose: Verify FetchYrtAlertsJob command wrapper behavior, dispatcher uniqueness
...
$testRunId = 'yrt_phase4_'.now()->format('Y_m_d_His');
...
try {
    ...
    deleteQueuedJobs(FetchYrtAlertsJob::class);
    ...
    try {
        ...
        assertTrue(
            str_contains($e->getMessage(), 'yrt:fetch-alerts failed with exit code 1'),
    ...
    assertTrue($job->uniqueId() === 'fetch-yrt-alerts', 'job uniqueId matches expected');
    assertTrue(count($middleware) === 1, 'job middleware contains one entry');
    assertTrue(($middleware[0]->key ?? null) === 'fetch-yrt-alerts', 'middleware key matches expected');
    ...
    $firstDispatch = $dispatcher->dispatchYrtAlerts();
    $secondDispatch = $dispatcher->dispatchYrtAlerts();

    assertTrue($firstDispatch === true, 'first dispatchYrtAlerts call enqueues job');
    assertTrue($secondDispatch === false, 'second dispatchYrtAlerts call is skipped');

    logInfo('Dispatcher idempotency verified from return values', [
        'first_dispatch' => $firstDispatch,
        'second_dispatch' => $secondDispatch,
        'observed_jobs_table_count' => queuedJobCount(FetchYrtAlertsJob::class),
    ]);
    ...
    assertTrue($event !== null, 'schedule contains yrt:fetch-alerts event');
    assertTrue($event->expression === '*/5 * * * *', 'yrt schedule runs every five minutes', [
        'expression' => $event->expression,
    ]);
    assertTrue($event->withoutOverlapping === true, 'yrt schedule uses withoutOverlapping');
    assertTrue($event->expiresAt === 10, 'yrt schedule overlap lock expiry is 10 minutes', [
