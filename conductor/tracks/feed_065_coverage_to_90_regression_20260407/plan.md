...
- [x] Task: Cover GTA allowlist failure path (422 after regex passes)
...
- [x] Task: Ensure `WeatherFetchException` maps to 503 when the allowlist check passes
    - Arrange:
...
- [x] Task: Cover queue name resolution branches (string, BackedEnum, UnitEnum)
    ...
    - For each queue type (string, BackedEnum, UnitEnum):
        ...
        - Arrange: dispatch a job instance to the database queue using a _string_ queue name:
