...
┌────────────────────────────────┐
│       go_transit_alerts        │
│  external_id · alert_type      │
│  service_mode · posted_at      │
└────────────────────────────────┘
```
...
All four alert tables share the same structural conventions:
...
All other tables have no automated pruning. Alert source tables (`fire_incidents`, `police_calls`, `transit_alerts`, `go_transit_alerts`) are append-only; rows are deactivated (soft-flagged) rather than deleted when they disappear from the feed.
