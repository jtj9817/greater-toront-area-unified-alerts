...
#### NotificationToastLayer.test.tsx (12 tests)

- Basic subscription behavior (2 tests)
    - Subscribes to authenticated user's private channel
    - Does not subscribe when no authenticated user
- Toast queue limits (1 test)
    - Enforces max display count of 4 toasts
- Invalid payload handling (2 tests)
    - Ignores invalid payloads with no render side-effects
    - Renders only valid payloads when mixed with invalid ones
- Auth user transitions (3 tests)
    - Transitions channel subscription when authUserId changes
    - Cleans up subscription when authUserId becomes null
    - Starts subscription when authUserId changes from null to a value
- Toast rendering details (4 tests)
    - Renders toast with correct severity styling
    - Renders source label with proper formatting
    - Renders dismiss button with accessible label
    - Allows manual dismissal of toasts

#### SettingsView.test.tsx (20 tests)

- Guest user behavior (1 test)
- Authenticated user preferences (1 test)
- Route options merge, deduplication, and sorting (3 tests)
- Alert-type dependent route disabling (3 tests)
- Geofence management (5 tests)
- API error handling (4 tests)
- Preference update behavior (3 tests)

#### App.test.tsx (9 tests)

- Original domain enforcement tests (4 tests)
- Toast layer mounting for authenticated users (2 tests)
- Channel cleanup on unmount (1 test)
- Auth user transitions (2 tests)
...
```bash
CI=true LARAVEL_BYPASS_ENV_CHECK=1 pnpm exec vitest run \
  resources/js/features/gta-alerts/components/NotificationToastLayer.test.tsx \
  resources/js/features/gta-alerts/components/SettingsView.test.tsx \
  resources/js/features/gta-alerts/App.test.tsx
```

All 41 tests pass successfully with no failures or timeouts.
