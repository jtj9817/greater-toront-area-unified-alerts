...
## Acceptance Criteria
- [x] Feed API contract test includes an explicit `source=yrt&status=cleared`
      assertion.
- [x] Assertion verifies returned records are `source=yrt` and `is_active=false`.
- [x] Existing assertions for `source=yrt`, `source=yrt&status=active`, and
...
- Resolution run:
  - `vendor/bin/sail artisan test --compact --filter='the feed api respects source and status filters for yrt without affecting existing sources' tests/Feature/Api/FeedControllerTest.php`
  - `vendor/bin/sail artisan test --compact tests/Feature/Api/FeedControllerTest.php`
...
- These fixes are part of Phase 6: Source Enum + Backend Contract Plumbing.
