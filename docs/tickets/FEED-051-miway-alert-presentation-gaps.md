...
- [x] Opening details for a MiWay alert does not throw.
- [x] `AlertDetailsView` always provides a defined `sections` payload to `AlertDetailsLayout`.
- [x] MiWay alerts display a non-empty source label in `AlertCard`.
- [x] MiWay alerts display a non-empty source label in `AlertTableView`.
- [x] Existing alert kinds (`fire`, `police`, `transit`, `go_transit`) keep current behavior.
