...
- [x] Opening details for a DRT alert does not throw and renders a valid details layout.
- [x] `AlertDetailsView` has a `drt` handling path or safe fallback guaranteeing `sections` is defined.
- [x] DRT source label renders in `AlertCard`.
- [x] DRT source label renders in `AlertTableView`.
- [x] Targeted frontend tests cover DRT details and source-label branches and pass.

## Resolution

- **P1 Fixed:** Added `buildDrtSections()` and `case 'drt'` in `AlertDetailsView.tsx` — teal-themed DRT service advisory header, route metadata, and DRT notice section. Test added.
- **P2 Fixed:** Added `case 'drt': return 'DRT'` in both `AlertCard.tsx` and `AlertTableView.tsx` `getSourceLabel` switches. Tests added for both.
- All 39 frontend test files pass (347 tests). All 942 backend tests pass. Pint, ESLint, Prettier clean.
