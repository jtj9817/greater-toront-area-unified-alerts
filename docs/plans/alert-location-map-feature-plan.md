...
- `resources/js/app.tsx`
  - Single frontend entry point; correct place to import Leaflet CSS once globally.
...
**Key Logic/Responsibilities**:
- Import Leaflet CSS globally from the app entry point rather than per-component.
...
- Optional assertion: preserve current `img-src 'self' data: https:` behavior and document that it remains sufficient for the chosen tile strategy.
...
## Execution Order
1. **Add dependencies and global CSS import** — establish the frontend runtime.
2. **Create shared Leaflet asset helper** — prevent broken marker icons before UI wiring begins.
3. **Preserve coordinates in presentation mapping** — unblock map rendering from current domain data.
4. **Create reusable `AlertLocationMap` and fallback UI** — isolate map behavior behind one component boundary.
5. **Refactor `AlertDetailsView` to use the shared location section** — replace the existing placeholder path.
6. **Add and update Vitest coverage** — lock in behavior for map-present and map-missing states.
7. **Run targeted frontend verification** — confirm no type, lint, or component regressions.
...
6. **StrictMode Double Mount**: Keep component setup idempotent and avoid custom imperative map initialization outside React Leaflet. 🔴
7. **Future Tile Provider Swap**: Keep the tile URL and attribution isolated in one component or config seam. 🔴
