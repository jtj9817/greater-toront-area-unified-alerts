...
#### Task 1.2: Load Leaflet CSS within the Map Component Bundle 🔴

**File**: `resources/js/features/gta-alerts/components/AlertLocationMap.client.tsx` (to be created)

**Key Logic/Responsibilities**:

- Import Leaflet CSS (`import 'leaflet/dist/leaflet.css';`) directly in the client-only map component rather than globally.
- Vite will automatically extract and lazy-load this CSS only when the map chunk is requested, saving ~40KB+ of CSS/JS for users who don't view an alert with a map.
...
```ts
// Basic bounding box check for the GTA/Ontario region to prevent "Null Island" (0,0) or bad data
const hasCoordinates =
    typeof alert.location?.lat === 'number' &&
    Number.isFinite(alert.location.lat) &&
    alert.location.lat > 40 &&
    alert.location.lat < 50 &&
    typeof alert.location?.lng === 'number' &&
    Number.isFinite(alert.location.lng) &&
    alert.location.lng > -90 &&
    alert.location.lng < -70;
...
export function AlertLocationMap({ lat, lng, label }: AlertLocationMapProps) {
    // Determine if device is mobile to prevent scroll-trapping
    const isMobile =
        typeof window !== 'undefined' &&
        window.matchMedia('(max-width: 768px)').matches;

    return (
        <div className="isolate z-0 h-72 overflow-hidden rounded-lg border border-white/10">
            <MapContainer
                key={`${lat}-${lng}`} // Forces remount on coordinate change, bypassing immutable center/zoom
                center={[lat, lng]}
                zoom={15}
                scrollWheelZoom={false}
                dragging={!isMobile} // Prevents touch-scroll trap on mobile devices
...
// Lazy load the Leaflet map and its CSS to optimize the main bundle
const AlertLocationMapClient = lazy(() => import('./AlertLocationMap.client'));

export function AlertLocationMap(props: AlertLocationMapProps) {
    return (
        <Suspense fallback={<AlertLocationMapSkeleton />}>
            <AlertLocationMapClient {...props} />
        </Suspense>
    );
}
```
        ...
        <div className="flex h-72 items-center justify-center rounded-lg border border-dashed border-white/10 bg-white/5 p-6 text-center">
            <div>
                ...
                <p className="mt-2 text-xs text-text-secondary">
                    A location label is available, but this alert does not
                    include coordinates yet.
                </p>
...
3. **Zero Coordinates & Null Island**: Reject `0,0` and wildly inaccurate geometries by enforcing a loose bounding box around the GTA (e.g., lat 40-50, lng -90 to -70) at the presentation boundary. 🔴
4. **Broken Default Marker Icons**: Configure Leaflet marker asset URLs explicitly for Vite. 🔴
5. **Map Container Height Collapse & Z-Index Bleed**: Give the map wrapper an explicit fixed height so the Leaflet canvas always initializes visibly, and use `isolate` to prevent Leaflet's aggressive z-indexes from bleeding over application tooltips or sticky headers. 🔴
6. **SSR / Hydration Mismatch**: Keep Leaflet behind a client-only render gate because this app ships an SSR entry. Code-splitting via `lazy()` efficiently achieves this. 🔴
...
9. **Mobile Scroll Hijacking**: Disable map panning (`dragging={!isMobile}`) on mobile touch devices to prevent users from getting "trapped" when trying to scroll the page. 🔴
10. **React-Leaflet Immutable Viewport**: Provide a unique `key` (e.g., `key={`${lat}-${lng}`}`) to the `MapContainer` to force a clean remount if coordinates update or the user switches alerts, avoiding the stuck map bug. 🔴
