...
## 2025-03-07 - FormRequest Array Input TypeError
**Vulnerability:** In FormRequests, using `prepareForValidation` to sanitize inputs (like `name`) without checking if the input is a string (e.g., using `strip_tags(trim($this->input('name')))` or casting like `(string) $this->input('name')`) causes a PHP `TypeError` (or Array to string conversion warning) when an array payload is provided (e.g. `?name[]=test`). This results in a 500 error instead of a graceful 422 validation failure.
**Learning:** Similar to Fortify Actions, FormRequests that manually modify input data via `merge` must explicitly check the type of the input before passing it to string functions like `strip_tags` and `trim`.
**Prevention:** Always wrap manual sanitization steps in type checks (`is_string()`) within `prepareForValidation` instead of blindly applying string functions or string casting.
